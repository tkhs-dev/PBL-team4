...
class Evaluator:
    ...
    def evaluate(self, game_state):
        return self.model(*map(lambda x:x.unsqueeze(0).to(device),self.get_input_tensor(game_state))).squeeze().tolist()
    ...
    def get_input_tensor(game_state : dict) -> (torch.Tensor, torch.Tensor):
        ...
        free_board = np.ones((6,6), dtype=np.integer)
        you = game_state["you"]
        for body in you["body"]:
            if 0 <= body["x"] < 6 and 0 <= body["y"] < 6:
                body_board[body["y"]][body["x"]] = 1
                free_board[body["y"]][body["x"]] = 0
        if 0 <= you["head"]["x"] < 6 and 0 <= you["head"]["y"] < 6:
            head_board[you["head"]["y"]][you["head"]["x"]] = 1
            free_board[you["head"]["y"]][you["head"]["x"]] = 0
        for food in game_state["board"]["food"]:
            food_board[food["y"]][food["x"]] = 1
            free_board[food["y"]][food["x"]] = 0
        one_hot_encoded = np.stack([body_board, head_board, food_board, free_board], axis=0)
